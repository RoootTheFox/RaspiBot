...
public class LeaveCommand implements Command {
    ...
    public void run(Message msg, String[] args, Guild guild) {
        ...
        if(args[0].equalsIgnoreCase("channel")) {
            ...
            String SQL_QUERY = "INSERT INTO server_data SET `server_id` = '"+msg.getGuild().getId()+"', `leave_channel` = '"+leaveChannel.getId()+"' ON DUPLICATE KEY UPDATE `server_id` = '"+msg.getGuild().getId()+"', `leave_channel` = '"+ leaveChannel.getId()+"'";
            try {
        ...
        if(args[0].equalsIgnoreCase("message")) {
            ...
            String SQL_QUERY = "INSERT INTO server_data SET `server_id` = '"+msg.getGuild().getId()+"', `leave_msg` = '"+text+"' ON DUPLICATE KEY UPDATE `server_id` = '"+msg.getGuild().getId()+"', `leave_msg` = '"+text+"'";
            try {
                Connection con = DataSource.getConnection();
                PreparedStatement pst = con.prepareStatement(SQL_QUERY);
                pst.executeQuery();
